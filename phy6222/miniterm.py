...
def main(default_port=None, default_baudrate=9600, default_rts=None, default_dtr=None, serial_instance=None, default_eol='CRLF'):
    ...
    group.add_argument(
        '--rtstoggle',
        type=int,
        help='set to true if RTS line should toggle',
        default=None)

    group.add_argument(
        '--rtsexit',
        type=int,
        help='set value of RTS to set when exiting',
        default=None)
    ...
    while serial_instance is None:
        ...
        try:
            ...
            if args.rts is not None:
                if args.rtstoggle is not None:
                    if not args.quiet:
                        sys.stderr.write('--- waiting {} ms then forcing RTS {}\n'.format(args.rtstoggle, 'inactive' if args.rts else 'active'))
                    time.sleep(args.rtstoggle / 1000)
                    serial_instance.rts = 0 if args.rts else 1
    ...
    try:
        ...
        pass
    if args.rtsexit is not None:
        if not args.quiet:
            sys.stderr.write('--- setting RTS {}\n'.format('active' if args.rts else 'inactive'))
        serial_instance.rts = args.rts
    if not args.quiet:
        sys.stderr.write('\n--- exit ---\n')
    miniterm.join()
